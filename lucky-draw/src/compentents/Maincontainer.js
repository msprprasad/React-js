...
const MainContainer =() =>{
  const[LuckyPerson,setLucky] = useState("");
...
function pickPerson()
{
  ...
  console.log('person'+people[randomMember].name);
  const person =people[randomMember];
  setLucky(person.name);
  console.log(`${person.name} from ${person.email}`);
}
    return (
      <>
        ...
        <div className='flex-center'>
          <h2>{LuckyPerson}</h2>
          <ButtonWraper text="Lucky Draw" onClick={pickPerson}/>  
        </div>
        <div>
          <p>{}</p>
        </div>
